...
## 2. สร้างตารางข้อมูล
ไฟล์ SQL ทั้งหมดรวมไว้ในไฟล์เดียวแล้วที่ **`supabase-all.sql`** เรียงตามลำดับที่ต้องรัน
(schema → migration ยุคแรก → security-hardening → migration ที่ Supabase CLI ดูแล
เรียงตาม timestamp → สคริปต์ utility ท้ายสุด) แต่ละส่วนมีคอมเมนต์ `-- FILE: ชื่อไฟล์` กำกับไว้

1. ในโปรเจกต์ Supabase ไปที่เมนู **SQL Editor** → New query
2. รัน `supabase-all.sql` ทั้งไฟล์ตามลำดับที่จัดไว้ — **ข้ามส่วน UTILITY ท้ายไฟล์ได้** เพราะเป็น
   สคริปต์วินิจฉัย/ซ่อมบำรุงไว้เรียกใช้เองเป็นครั้ง ๆ ไม่ใช่ขั้นตอนติดตั้ง

> `supabase-all.sql` เป็นไฟล์ที่ **สร้างอัตโนมัติ ห้ามแก้ด้วยมือ** — เพิ่ม migration ใหม่ที่
> `supabase/migrations/` แล้วรัน `npm run build:sql` (หรือ `npm run check` ซึ่งรันให้อยู่แล้ว)
> ไฟล์ต้นทางส่วนที่แช่แข็งอยู่ที่ `supabase/legacy/` ใช้ `npm run check:sql` เช็คว่าไฟล์ตรงกับต้นทางไหม
> ส่วนฐานข้อมูลที่ตั้งไว้แล้วให้ใช้ `supabase db push` ตามปกติ ไม่ต้องแตะไฟล์นี้
