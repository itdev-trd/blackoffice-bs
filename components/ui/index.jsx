...
// โทเคนสี/รัศมี/เงา อยู่ใน app/globals.css (:root + .ds-*)
// นำเข้าใช้: import { Button, Card, StatCard, ... } from "@/components/ui";
// =====================================================================
import { Loader2 } from "lucide-react";

// พาเลตต์ accent (ตรงกับ --token ใน app/globals.css) สำหรับ tile/แถบสีที่ต้องเป๊ะ
export const TONE = {
  purple: "#9D6BFF",
  gold: "#F7C948",
  green: "#1ED760",
...
export function Card({ glass = false, hover = false, className = "", children, ...props }) {
  return (
    <div className={cx(glass ? "ds-card-glass" : "ds-card", hover && "ds-hover-lift", className)} {...props}>
      {children}
    </div>
  );
}
...
export function StatCard({ icon: Icon, label, value, sub, tone = "purple", delta, onClick }) {
  ...
  return (
    <div
      ...
      className={cx("ds-card p-5 flex flex-col gap-3 relative overflow-hidden", clickable && "ds-hover-lift cursor-pointer focus:outline-none")}
...
export function Badge({ tone = "purple", children, className = "" }) {
  const color = TONE[tone] || TONE.purple;
  return (
    <span className={cx("inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full", className)} style={{ background: `${color}1f`, color }}>
      {children}
    </span>
  );
}
...
export function Toggle({ checked, onChange, label }) {
  return (
    <button type="button" role="switch" aria-checked={!!checked} onClick={() => onChange?.(!checked)} className="inline-flex items-center gap-2.5 group">
      <span className="relative w-10 h-6 rounded-full transition-colors" style={{ background: checked ? TONE.purple : "rgba(255,255,255,.12)" }}>
        <span className="absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform" style={{ transform: checked ? "translateX(16px)" : "none" }} />
      </span>
