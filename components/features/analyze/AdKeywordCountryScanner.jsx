...
export default function AdKeywordCountryScanner({ restricted = false, allowedAccounts = [] }) {
  ...
  return (
    <section className="analysis-scanner ds-card p-4 sm:p-5 space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2"><span className="analysis-step">01</span><Globe2 size={19} className="text-blue-500" /><h2 className="text-base sm:text-lg font-semibold">ตรวจประเทศโฆษณาตามคีย์เวิร์ด</h2></div>
      ...
      <div className="analysis-scanner-form grid gap-3 md:grid-cols-[minmax(220px,0.8fr)_minmax(0,1.5fr)_auto] items-end">
      ...
      {result && <div className="space-y-3">
        ...
        <div className="analysis-country-summary grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{result.summary?.length ? result.summary.map((row) => <div key={row.country_code} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="flex items-center justify-between gap-2"><span className="font-medium text-slate-800">{countryLabel(row.country_code)}</span><span className="text-xs font-semibold text-blue-600">{row.matched_ads} แอด</span></div><div className="text-[11px] text-slate-500 mt-1">{row.country_code} · {row.keywords.join(", ")}</div></div>) : <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-500 sm:col-span-2 lg:col-span-4">ไม่พบประเทศที่ระบุเป็นรหัสประเทศตรงๆ</div>}</div>
